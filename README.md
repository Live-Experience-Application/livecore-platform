...
[![Packages](https://img.shields.io/badge/packages-0.5.0-blue.svg)](CHANGELOG.md)
...
scripts/LiveCoreStorageDocsLint.psm1  storage/self-hosting docs RustFS backstop: each doc names RustFS (the default example S3-compatible backend), carries no leftover MinIO setup guidance and keeps the any-S3-compatible contract stated (CORE-STOR-002)
scripts/lint-storage-docs-rustfs.ps1  CI grep backstop that fails the build on any remaining MinIO setup/console/credential guidance in the storage/self-hosting docs after the RustFS migration (CORE-STOR-002)
scripts/test-storage-docs-rustfs.ps1  tests for the RustFS docs backstop (a doc still naming MinIO fails; a clean RustFS doc passes; the real docs pass — CORE-STOR-002)
...
(CORE-DEP-006) adds **Keycloak** (OIDC), **RustFS** (the default example
S3-compatible object store — any S3-compatible provider works, ADR 0006) and
**Valkey** (the realtime backplane) and pre-wires the API/worker to them, while
