...
/// transaction as the recording (reusing the CORE-CONC-002 unit of work), exactly as the Apple endpoint does: the
/// buyer's user subject is resolved server-side (their <c>users(id)</c> profile, provisioned on first sight) and a
...
///   resolve here (CORE-STORE-002: <c>purchase_transactions</c> has no <c>organization_id</c>); the isolation that
///   matters is per-SUBJECT — one buyer's purchase can never be claimed through another subject's identity (the
///   buyer linkage's uniqueness rule).</item>
...
///   <item>A genuine purchase is recorded, linked to the buyer and returned 200; a rejected token is 422 (a
///   semantically invalid command — the submitted token is not a grantable purchase) carrying only the generic,
///   log-safe rejection reason, with nothing recorded; a genuine purchase already claimed by a different subject
///   is 409, granting this caller nothing.</item>
/// </list>
///
...
internal static class GooglePurchaseEndpoints
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var idempotencyScope = $"purchase-google:{profile.Id}";
