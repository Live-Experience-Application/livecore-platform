...
/// directly on <see cref="ExportManifestView"/> / <see cref="ExportManifestSummaryView"/>), so the wire
/// contract uses the name exactly as the other Core enums do (docs/08_API_CONTRACTS.md "enums as stable
/// string names"). The converter is attached to the enum itself because it is surfaced directly (rather than
/// pre-mapped to a string in its DTO), mirroring <see cref="LiveCore.Api.Audit.AuditAction"/>; the EF string
/// conversion is configured separately, so this attribute affects JSON only.
/// </summary>
