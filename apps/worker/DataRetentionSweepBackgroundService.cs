...
internal sealed class DataRetentionSweepBackgroundService : BackgroundService
{
    ...
    private async Task RunSweepAsync(CancellationToken stoppingToken)
    {
        ...
        // Distributed-tracing span for this irreversible-purge iteration (CORE-OBS-012). Tagged with the coarse
        ...
        try
        {
            ...
            // sweep duration and the observed backlog/queue depth (the total candidate records it examined across
            // every family) — so a loop falling behind is alertable. Counts only, tagged by the coarse loop
            // name, never a tenant/principal/content detail (threat T7).
            ...
            _metrics.RecordBackgroundJobBacklog(_jobName, result.TotalExamined);
