...
internal sealed class ExportProcessingBackgroundService : BackgroundService
{
    ...
    private async Task RunSweepAsync(CancellationToken stoppingToken)
    {
        ...
        // Distributed-tracing span for this export-processing iteration (CORE-OBS-012). Tagged with the coarse
        ...
        try
        {
            ...
            // sweep duration and the observed backlog/queue depth (the count of pending jobs it examined) — so a
            // loop falling behind is alertable. Counts only, tagged by the coarse loop name, never a
