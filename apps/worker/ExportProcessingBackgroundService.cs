...
///
/// <para>
/// Liveness heartbeat (CORE-OPS-005): like the other worker loops, this loop writes a
/// <see cref="WorkerHeartbeat"/> on startup and after every sweep tick, so orchestration can detect a WEDGED
/// loop (its heartbeat file goes stale when a sweep hangs). The loops share the one worker-process heartbeat
/// (the heartbeat is the process's liveness signal, refreshed whenever any loop makes progress).
/// </para>
/// </summary>
internal sealed class ExportProcessingBackgroundService : BackgroundService
{
    private const string _jobName = "export-processing";
