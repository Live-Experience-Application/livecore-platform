/**
 * Publish-shape / packed-tarball tests for @livecore/sdk-ts (CORE-PUB-001).
 *
 ...
 * stays clean. pnpm also rewrites the `workspace:*` dependency on @livecore/contracts
 * to the resolved version in the packed manifest, so the published tarball carries no
 * `workspace:` protocol. No new dependency: `pnpm pack --json` reports the file
 * manifest directly, so the test never has to open the gzip/tar itself.
 */
...
test("the workspace dependency is still a workspace link in the manifest (CORE-PUB-001)", () => {
  ...
  // live package, not a registry copy.
  assert.equal(manifest.dependencies["@livecore/contracts"], "workspace:*");
});
