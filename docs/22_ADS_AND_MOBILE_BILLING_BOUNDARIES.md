...
- **Out of scope (later stories).** A temporary per-session (rewarded-ad) ad-free window that would populate
  ...
  The verified-purchase → `ads.disabled` grant is part of the purchase-to-entitlement chain that is **in scope for
  Core v1** (billing/monetization is in scope, `docs/01_PRODUCT_VISION_AND_SCOPE.md`; the v1 scope and acceptance
  are recorded in `docs/24_SPEC_CONSISTENCY.md`, and the grant chain is built by CORE-MON-001..010 — CORE-MON-001
  reversed the earlier CORE-DOC-002 post-v1 deferral). Until that grant lands, this read reflects only entitlements
  assigned by other means, and a subject with no ad-free grant fails closed to ads-required.
