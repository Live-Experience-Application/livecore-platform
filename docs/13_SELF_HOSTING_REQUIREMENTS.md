...
| `Tracing:Otlp:Endpoint`             | `Tracing__Otlp__Endpoint`          |   no   | for trace export        | API, worker | Spans produced but not exported (no collector); the worker exports its per-loop job spans on the same key (CORE-OBS-003, CORE-OBS-012) |
