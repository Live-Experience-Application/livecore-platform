...
| `push_subscriptions.endpoint`, `.p256dh`, `.auth` (FK `user_id`→`users`, `CASCADE`) | global | A principal's browser Web Push subscription: the push service `endpoint` URL plus the client `p256dh` public key and `auth` encryption secret — per-device personal data the subject registers (CORE-PUSH-001). | `CASCADE` on erasure: the subject's subscriptions are removed with their profile. **Disclosed** in the access/portability export (CORE-PRIV-004) as the subscription id, endpoint and creation time only — the `p256dh`/`auth` keys are **never** projected (threat T7). A push endpoint that returns `404`/`410` (gone) also deletes the subscription as a normal lifecycle reclaim (CORE-PUSH-002). |
| `push_notification_deliveries.user_id` (FK→`users`, `CASCADE`), `.organization_id` (FK→`organizations`, `CASCADE`) | tenant | A **transient** closed-app push outbox row: the recipient and tenant, plus the source `session_event_id`/`session_id` it signals — **identifiers only, no projected content** (CORE-PUSH-002, threats T2/T7). | `CASCADE` on erasure and on tenant teardown: a subject's / tenant's pending pushes are removed with the profile / tenant root. The worker also **deletes** each row as it drains the outbox, so rows are short-lived. The delivered push is content-free (a payload-less signal) — nothing sensitive can surface on a lock screen. |
